...
local function newStats(mutate, overrides)
    ...
    -- THE ONE DATABASE GATE, and it lives here now. ArenaDb/ArenaDbReady --
    -- and the missing-oxmysql warning the tests at the bottom of this file
    -- count -- used to be stats.lua's own. They were consolidated into
    -- server/util.lua, which fxmanifest.lua loads FIRST of the server
    -- scripts, so it is loaded first here too.
    Sandbox.loadInto('../server/util.lua', env)

    -- LAYERED AFTER util.lua, not before it. util.lua defines the real
    -- ArenaLog/ArenaDebug, so an override applied ahead of the load is
    -- overwritten by it and a fixture that captures the console captures
    -- nothing.
