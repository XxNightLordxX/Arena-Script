...
t.test('every admin event: a non-admin is refused before any module is touched', function()
    ...
    for _, name in ipairs(ADMIN_EVENTS) do
        s.fire(name, 2, { matchId = 'm-1', target = 3, citizenid = 'CID003', stash = 's',
            forced = 'open', force = true, confirm = true })
...
t.test('exactly the 26 wire events STAGE 3 counts are registered, plus the one callback', function()
    local s = newFunnel()
    local names = {}
    ...
    table.sort(names)
    t.equals(#names, 26, table.concat(names, ' '))
    ...
    for _, name in ipairs({ 'panelClosed', 'requestState', 'outlineReason', 'createMatch', 'joinMatch',
                            ...
                            'adminRevive', 'adminTool', 'adminUnjam', 'adminWipe', 'clientDebug' }) do
