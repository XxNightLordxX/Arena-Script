...
t.test('DEFECT: a forfeited stake that was handed back is counted and logged as kept', function()
    -- The console said the arena "kept N forfeited stake(s) rather than
    -- refunding them" about stakes it had just refunded, and the returned
    -- totals left the money out. Nothing was paid wrongly -- this is the
    -- record of what happened being the opposite of what happened.
    local server, record = teamMatch(nil, function(config)
        config.Betting.refundOnDisconnectBeforeStart = false
    end)

    -- 2 walks out and their fee is forfeited into the pot.
    t.equals(server.betting.KeepInPot('m1', 2), 1000)
    record.players[2] = nil

    local before = server.cash(2)

    -- The lobby is torn down. NOT an admin stop, which is the case that
    -- breaks: every reason except an admin stop still hands the forfeit back,
    -- because the clear inside RefundAll drops the flag before RefundOne can
    -- refuse on it.
    local settled, refunded, total = server.betting.RefundAll('m1', 'notify.match_cancelled')

    t.isTrue(settled, 'the refund did not complete, so this proves nothing')
    t.equals(server.cash(2), before + 1000,
        'the forfeited stake was not actually handed back, so this proves nothing')

    t.equals(refunded, 2, 'the forfeited stake was handed back but not counted as refunded')
    t.equals(total, 2000, 'the refunded total left out the forfeited stake it had just paid')
