...
--- The same question asked on a FIGHTER's behalf.
---
--- The snapshot is built once and sent to everybody, so it cannot ask this
--- per viewer -- but the answer differs by viewer, and the panel has to be
--- told both or it lies to half the room. The book shuts for a fighter the
--- instant the round goes live and stays open to watchers for the grace
--- window, so a fighter was shown a lit Place Bet button through the whole of
--- it and got "Book is closed on this one." on every click. Exactly the
--- defect BetsAreOpen was added to fix, reintroduced for the other half of
--- the room by the guard that closed the fighters' book.
--- @param match table
--- @return boolean
