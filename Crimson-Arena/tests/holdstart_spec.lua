...
t.test('DEFECT: a host could hold and restart the countdown for ever, and nobody could leave', function()
    -- Leave is refused during a countdown so that one player cannot call off
    -- every round by standing up. The host could turn that into a stall:
    -- Start, Hold, Start, Hold -- every player held in a lobby they could not
    -- leave, taking a toast on every cycle, for as long as the host clicked.
    local server, matchId = counting(0, slowLobby)

    -- One hold and a restart is the honest use of the button and stays free.
    server.lobby.HoldCountdown(1)
    server.fire('startMatch', 1)
    t.equals(server.lobby.Get(matchId).state, 'countdown',
        'a single hold and restart was refused -- that is the team-switch flow, and it must work')

    -- The second hold in quick succession is the stall, and the restart
    -- after it is turned away long enough for everybody to get out.
    server.lobby.HoldCountdown(1)
    t.equals(server.lobby.Get(matchId).state, 'lobby', 'the second hold did not land, so this proves nothing')
    server.fire('startMatch', 1)

    t.equals(server.lobby.Get(matchId).state, 'lobby',
        'the host restarted the countdown straight after holding it a second time')
