...
| `ArenaMatch.RememberDamage(victimSrc, attackerSrc)` | The server watched one fighter land a hit on another; remember it for five seconds. Called once per landed hit from `server/dispatch.lua`'s `weaponDamageEvent` handler, which is the only place this resource sees a bullet. `OnDeath` reads it ONLY when the dying client named nobody on the roster -- a claim naming a fighter in the round keeps its own answer, refusal included -- and runs what it names through the same `resolveKiller` checks as any claim. It can credit only the LAST fighter to land a hit; beside that it keeps when each fighter last landed one, which a gun game asks when the victim names a killer who has already taken `maxTiersPerVictim` off them -- that death is spared only if the server saw THAT killer land a hit in the five seconds. Hits on a fighter the round has already booked as dead are not recorded. Hits on a body the dead sweep is still counting are, but a death the sweep has already read dead is judged on the copy it took at that reading -- whether the sweep books it or a report naming nobody on the roster arrives afterwards -- so they cannot take that kill. No copy is taken off a respawned body the sweep has not yet seen standing, and a copy kept through more than two unreadable readings in a row is dropped. The packet is the shooter's own client talking, so a modified client can plant an entry; the roster, team, fence, distance, alive and five-second checks bound what that buys. Stores a fact, never a verdict. |
| `ArenaMatch.OnDeath(src, killerSrc, serverSaw, why, causeHash, witnessed)` | One player died. `serverSaw` marks a death the dead-sweep found rather than the client reporting; `witnessed` is that sweep's copy of the damage memory, taken at the check that FIRST read the body dead, and a death the sweep books reads nothing else -- so it DOES credit the shooter, the last fighter the server saw land a hit in the five seconds before that check (not before the booking seconds later), through the same `resolveKiller` checks as any claim, and credits nobody when that copy is empty or too old, whoever shot the corpse during the count (nil from every other caller; a report that arrives after the sweep has taken a copy is judged on that copy too -- its credit when it names nobody on the roster, and a capped gun game's sparing -- and one that arrives before, or while a respawned body has not yet been seen standing, reads the memory as it stands); `why` is for the log alone; `causeHash` is the weapon hash the dying client read off its own ped -- the only way the server can name what killed somebody, used to print the TEAMKILL line and to revoke a sparing the killer's rung would otherwise have earned. In a gun game a gun kill spares the victim's tier only when it moved a tier to the killer, or -- past `maxTiersPerVictim` -- when the server saw that killer land a hit in the last five seconds. |
