...
t.test('and NOTHING reads the field behind its back', function()
    ...
    for _, name in ipairs(files) do
        local handle = assert(io.open('../Crimson-Arena/' .. name, 'r'), name .. ' is not where this spec expects it')
        local line = 0
...
-- ======================================================================
-- THE WIN CONDITION IS CHECKED AT BOOT, AS config.lua PROMISES
-- ======================================================================

t.test('DEFECT: a misspelt win condition ran every round under a rule nobody picked, silently', function()
    local said = validate(function(config) config.Match.winCondition = 'most_kils' end)
    t.contains(said, 'most_kils', 'the console does not name the misspelt value')
    t.contains(said, 'last_standing', 'the console does not say what the server is running instead')
end)

t.test('and the same for the default inside the table shape', function()
    local said = validate(function(config)
        config.Match.winCondition = { allowChoose = true, default = 'lastStanding' }
    end)
    t.contains(said, 'lastStanding', 'a wrong default inside the table went unnamed')
end)

t.test('and a value that is not a string or a table at all', function()
    local said = validate(function(config) config.Match.winCondition = 42 end)
    t.contains(said, 'winCondition', 'a number for the win condition went unnamed')
end)

t.test('CONTROL: the shipped win condition is not complained about', function()
    local said = validate()
    t.notContains(said, 'winCondition', 'the shipped config is being warned about')
end)

t.test('CONTROL: a table with no default is "use the default", not an error', function()
    local said = validate(function(config) config.Match.winCondition = { allowChoose = true } end)
    t.notContains(said, 'winCondition', 'leaving the default out was treated as a mistake')
end)
