...
t.test('a death reported from outside the arena is refused', function()
    -- THE FAKE-DEATH FARM. A death report costs the reporter nothing and
    -- hands whoever they name a kill, so a client sat out of the fight
    -- could feed an accomplice one every respawn delay. Being in the arena
    -- is the cheapest thing that can be demanded of it.
    ...
    t.equals(server.rowOf(2).deaths or 0, 0, 'the death was booked from outside the arena')
    t.equals(server.rowOf(1).kills or 0, 0, 'a kill was credited for a death outside the arena')
    t.isTrue(server.rowOf(2).alive == true, 'the reporter was knocked out by their own claim')
end)
