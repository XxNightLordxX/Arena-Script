...
-- ======================================================================
-- THE RULES LOCK COUNTED THE WRONG STAKES
-- ======================================================================

t.test('DEFECT: a host alone in a lobby on the shipped fee could not edit it', function()
    -- No entryFee in the request: Arena.ResolveEntryFee falls to the shipped
    -- default of 500, Create takes the host's own stake, and the lock read
    -- that pot as "people have already paid". The only person who had paid
    -- was the one being refused.
    local server = newArena({ [1] = 50000 })
    server.fire('createMatch', 1, { arenaKey = 'trailerpark', modeKey = 'ffa' })
    local match = server.lobby.All()[1]
    t.isTrue(server.betting.GetPot(match.id) > 0, 'no fee was taken, so this proves nothing')

    server.fire('updateMatch', 1, { lives = 5 })
    t.equals(server.lobby.Get(match.id).lives, 5,
        'the host was refused an edit on a lobby nobody but them has paid into')

    server.fire('updateMatch', 1, { arenaKey = 'skydome' })
    t.equals(server.lobby.Get(match.id).arenaKey, 'skydome',
        'the host could not move their own empty lobby to another arena')
end)

t.test('DEFECT: the mode escaped the entry-pot lock, so a host swapped the game after others had paid', function()
    ...
    local match = server.lobby.All()[1]
    server.fire('joinMatch', 2, { matchId = match.id })
    t.equals(server.cash(2), 49000, 'the second player did not pay, so this proves nothing')

    server.fire('updateMatch', 1, { modeKey = 'gungame' })

    t.equals(server.lobby.Get(match.id).modeKey, 'tdm',
        'the host rewrote what game is being played after somebody else had paid to play it')
end)

t.test('and once a second person has paid, the other rules still lock', function()
    -- The lock must not have been loosened, only re-aimed.
    local server = newArena({
        [1] = 50000,
        [2] = 50000,
    })
    ...
    server.fire('updateMatch', 1, { lives = 5 })
    t.equals(server.lobby.Get(match.id).lives, 3, 'the lives lock came off with the fix')
end)

t.test('DEFECT: a gun-game Apply that changed nothing was refused, because the plan was compared by identity', function()
    local server = newArena({ [1] = 50000 })
    server.fire('createMatch', 1, { arenaKey = 'trailerpark', modeKey = 'gungame', entryFee = 500 })
    local match = server.lobby.All()[1]
    local plan = server.lobby.Get(match.id).tierPlan

    -- A fresh table with the same numbers, which is what the panel posts.
    local same = {}
    for key, count in pairs(plan or {}) do same[key] = count end
    server.fire('joinMatch', 1, { matchId = match.id })

    local ok, reason = server.lobby.UpdateMatch(1, { tierPlan = same, lives = 5 })
    t.isTrue(ok, ('an Apply with an identical ladder was refused: %s'):format(tostring(reason)))
    t.equals(server.lobby.Get(match.id).lives, 5, 'the edit did not land')
end)


t.test('DEFECT: a rules rewrite left everybody Ready for a match they had not seen', function()
    local server = newArena({ [1] = 50000, [2] = 50000, [3] = 50000 })
    server.fire('createMatch', 1, { arenaKey = 'trailerpark', modeKey = 'ffa', entryFee = 0 })
    local match = server.lobby.All()[1]
    server.fire('joinMatch', 2, { matchId = match.id })
    server.fire('joinMatch', 3, { matchId = match.id })
    -- The host and ONE other ready up. Not all three: the last Ready would
    -- auto-start the round, and a rewrite of a counting-down lobby is
    -- refused for a different and correct reason.
    server.fire('setReady', 1, { ready = true })
    server.fire('setReady', 2, { ready = true })
    t.isTrue(match.players[1].ready and match.players[2].ready, 'nobody readied, so this proves nothing')
    t.equals(server.lobby.Get(match.id).state, 'lobby', 'the lobby auto-started, so the rewrite cannot land')

    server.fire('updateMatch', 1, { modeKey = 'gungame' })
    t.equals(server.lobby.Get(match.id).modeKey, 'gungame', 'the rewrite did not land')

    t.isFalse(match.players[2].ready == true, 'player 2 is still Ready for a different match')
    t.isTrue(match.players[1].ready == true, 'the host, who wrote the change, was un-readied too')
end)

t.test('and an Apply that changes nothing takes nobody\'s Ready back', function()
    local server = newArena({ [1] = 50000, [2] = 50000 })
    server.fire('createMatch', 1, { arenaKey = 'trailerpark', modeKey = 'ffa', entryFee = 0, lives = 3 })
    local match = server.lobby.All()[1]
    server.fire('joinMatch', 2, { matchId = match.id })
    server.fire('setReady', 2, { ready = true })

    server.fire('updateMatch', 1, { arenaKey = 'trailerpark', modeKey = 'ffa', lives = 3 })

    t.isTrue(match.players[2].ready == true, 'a no-op Apply un-readied a player')
end)
