--[[
    ...
    loadInto look like '../Crimson-Arena/shared/arena.lua'.
]]
...
--- Reads locales/en.json, once per process.
---
--- ITS OWN FUNCTION, AND NOT A LOOKUP OF SOME KEY. localeKeys below used to
--- force this load by asking for 'meta.locale_probe', a key that existed in
--- the file for no other reason -- so the loader depended on a string the
--- resource itself never used, and the day that dead key was deleted the
--- coverage spec stopped running at all. A load is a load; DO NOT express it
--- as a lookup again.
--- @return table
local function loadLocale()
    if localeDict then return localeDict end

    local handle = assert(io.open('../Crimson-Arena/locales/en.json', 'r'),
        'sandbox: could not open ../Crimson-Arena/locales/en.json -- specs run with crimson_arena/tests as the working directory')
...
--- An ox_lib-shaped `locale()`. Unlike ox_lib -- which hands back the key
--- itself when a key is missing, so the mistake shows up only as odd text
--- in-game -- this RAISES. Every spec that runs a code path through locale()
--- therefore doubles as proof that the key really exists.
function Sandbox.locale(key, ...)
    local dict = loadLocale()
    local group, leaf = key:match('^([^.]+)%.(.+)$')
    local value = group and dict[group] and dict[group][leaf] or dict[key]
...
function Sandbox.localeKeys()
    local flat = {}
    for group, value in pairs(loadLocale()) do
...
local NATIVE_DEFAULTS = {
    ...
    --- WHAT THE SERVER CALLS A CONNECTED PLAYER, and '' is the truthful
    --- answer for a fixture that models no names: ArenaPlayerName reads a
    --- blank as "not on this server" and falls through to its unknown-player
    --- string, which is exactly what a src with no player behind it is.
    ---
    --- Defaulted here rather than in each spec because a fixture missing it
    --- does not fail an assertion -- ArenaPlayerName THROWS, out of the
    --- middle of whatever console line asked for a name, and the spec that
    --- broke is never about names. A fixture that DOES model them overrides
    --- this, the same as every other native here.
...
--- @param path string -- relative to tests/, e.g. '../Crimson-Arena/fxmanifest.lua'
...
--- @param manifest table -- from readDeclarations('../Crimson-Arena/fxmanifest.lua')
...
function Sandbox.newQbxCore(players, opts)
    ...
    -- ANOTHER RESOURCE ON THE SAME EVENT, which is the shape this fixture
    -- could not express and which hid a defect that paid people twice.
    --
    -- AddMoney and RemoveMoney fire the framework's money-changed event, and
    -- a tax, bank or paycheck script hanging off it moves the SAME account in
    -- the same instant. So a balance re-read after a successful credit can be
    -- SHORT of the amount for a reason that has nothing to do with the credit
    -- -- and the arena used to read that as "the payment failed" and hand the
    -- money over again, every sweep, for as long as the player stayed online.
    --
    -- `rake` is that other resource: a flat amount taken back out of the
        ...
        return {
            ...
            Functions = {
                ...
                AddMoney = function(account, amount, reason)
                    ...
                    fake.ledger[#fake.ledger + 1] = { id = serverId, account = account, delta = amount, reason = reason }
                    if rake > 0 then
                        local taken = math.min(rake, record.money[account] or 0)
                        record.money[account] = (record.money[account] or 0) - taken
                        fake.ledger[#fake.ledger + 1] = {
                            id = serverId, account = account, delta = -taken, reason = 'rake',
                        }
                    end
                    return yes()
                end,
