...
local function newServer(opts)
    ...
    local env = Sandbox.newArenaEnv({
        ...
        -- An admin when a test asks for one, so the admin handler BODIES can be
        -- fuzzed too -- with the gate up they were never entered at all.
        IsPlayerAceAllowed = function() return (opts or {}).admin == true end,
...
t.test('the surface this file fuzzes is the whole surface, not just net events', function()
    ...
    for _, required in ipairs({
        ...
        -- And the one that takes the door's own safety catch off a stash. A
        -- hold cleared over contents nobody settled hands the owner a second
        -- copy of everything they carry, which makes it the admin event whose
...
t.test('the admin handler BODIES survive the corpus too, not just the gate in front of them', function()
    -- Every hostile payload above was turned away before the body: sources
    -- 0 and -1 by the rate limiter, 1/2/3/999 by the admin gate. So for the
    -- five admin events the sweep proved the gate holds and nothing about the
    -- code past it. A compromised admin client sends rubbish too.
    local server = newServer({ admin = true })
    local matchId = seedMatch(server)
    local result = sweep(server, matchId, { 1, 2, 3, 999 })
    t.isTrue(result.calls > 0, 'nothing was fuzzed, so this proves nothing')
    t.equals(#result.raised, 0,
        'an admin handler body raised on a hostile payload:\n' .. table.concat(result.raised, '\n'))
end)
