...
t.test('a tier weapon arrives with the rounds that do not fit in it', function()
    ...
    --
    -- ASKED OF THE EXIT, NOT OF A COUNTER. ArenaAmmo.OnLoan summed a second
    -- scalar ledger that four issue paths maintained and nothing decremented
    -- -- it reported cumulative-issued, never outstanding -- and 2fec9fe
    -- deleted the function and the ledger together. What the exit actually
    -- bills from is `issuedAmmo`, per item, and that is a file-local: it
    -- cannot be read from out here and re-deriving it in the fixture would
    -- assert against a copy. So this drives the reclaim and reads the
    -- pockets, which is the thing the counter was only ever a proxy for.
    s.ammo.Reclaim(1, 'test')
    t.equals(s.ox.count(1, entry.ammoTypeItem), 0,
        'the loose rounds were not on the arena\'s books, so the exit left them with the player')
end)
...
t.test('and the rounds it pays are on the arena\'s books', function()
    ...
    -- match at a time.
    --
    -- THE ONLY TEST ANYWHERE THAT COVERS THE KILL REWARD'S LEDGER WRITE.
    -- Deleting the `issuedAmmo` write in ArenaAmmo.GrantRounds and running
    -- all 77 spec files and all 7 harnesses changes NOTHING except this
    -- test -- the tier-issue write next door is caught by eight assertions
    -- in three files, and this one by none. DO NOT delete it as a duplicate.
    --
    -- ASKED OF THE EXIT, NOT OF A COUNTER, for the reason given on the tier
    -- test above: ArenaAmmo.OnLoan and the scalar ledger behind it went in
    -- 2fec9fe, and `issuedAmmo` -- what the exit really bills from -- is a
    -- file-local this cannot reach. The pockets after a reclaim are the same
    -- fact, observed where it matters.
    local s = newServer(function(config)
        config.Modes.ffa.killAmmo = 100
    end)
    s.play(3, 'ffa', function()
    ...
    local item = s.row(1).loadout.weapons[1].ammoTypeItem
    t.isTrue(s.arena.IsKey(item), 'the tier entry names no ammo item, so this proves nothing')

    local before = s.ox.count(1, item)
    s.kill(2, 1)
    t.equals(s.ox.count(1, item), before + 100, 'the kill reward never reached them')

    -- AND THE DOOR IS OFF IN THIS FIXTURE, so everything in these pockets is
    -- the arena's and the floor underneath it is zero. Anything still there
    -- after the reclaim is something the arena handed over and did not write
    -- down.
    s.ammo.Reclaim(1, 'test')
    t.equals(s.ox.count(1, item), 0,
        'the reward was handed over without being written down, so the exit could not ask for it back')
end)
