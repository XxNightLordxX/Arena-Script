...
files['client/'] = {
    ...
    read_globals = {
        ...
        -- The marker over a teammate's head. Base game natives, all of them
        -- older than FiveM: a draw origin turns ordinary 2D interface draws
        -- into ones positioned at a world point, with no depth test -- which
        -- is how the marker is seen through cover. IS_ENTITY_ON_SCREEN is
        -- the frustum test that keeps it off a teammate BEHIND the camera.
        -- See the MARKER_ block in client/match.lua.
        'SetDrawOrigin',
        'ClearDrawOrigin',
        'DrawRect',
        'IsEntityOnScreen',
