...
-- somebody added one.
local SOURCE_DIRS = { '../Crimson-Arena/client', '../Crimson-Arena/server', '../Crimson-Arena/shared' }
...
--
-- EMPTY, AND THAT IS THE HEALTHY STATE. Its one entry was
-- 'meta.locale_probe', a key that existed only so the sandbox could force
-- en.json to load by asking for something. bbd230e deleted the key and the
-- fixture forces the load directly now, so nothing is excused: every key in
-- the file is named by a file under SOURCE_DIRS. Add an entry only with a
-- reason, and the test below will hold that reason to being true.
...
t.test('the scan reached the source it is meant to police', function()
    ...
    -- AND THE FIXTURE'S OWN READER WORKS, which is what every other
    -- assertion in this file is standing on. A real key, named by
    -- server/util.lua -- not a probe key kept in en.json for the test's
    -- benefit, which is what this used to be.
    t.isTrue(Sandbox.locale('meta.unknown_player') ~= nil)
end)
...
t.test('every error.* reason shared/arena.lua returns exists in en.json', function()
    ...
    local code = stripComments(readFile('../Crimson-Arena/shared/arena.lua'))
