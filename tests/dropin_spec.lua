...
local function readManifest()
    ...
    local chunk = assert(loadfile('../Crimson-Arena/fxmanifest.lua', 't', env))
    chunk()
    return declared
end
...
local function fileExists(path)
    local handle = io.open('../Crimson-Arena/' .. path, 'r')
...
t.test('stats go through the oxmysql export, so the database is a run-time question', function()
    ...
    -- THE GATE MOVED, THE RULE DID NOT. stats.lua carried its own copy of
    -- "is oxmysql started" until it had drifted from ammo.lua's; both are
    ...
    t.contains(stats, 'ArenaDb(',
        'server/stats.lua no longer sends its queries through the one gate that does that check')

    -- A SECOND REACH THAT SKIPS THE GATE is the thing this test exists to
    -- catch. stats.lua must not touch the export itself.
    for line in stats:gmatch('[^\n]+') do
        if line:find('exports%.oxmysql') and not line:find('^%s*%-%-') then
            error('server/stats.lua reaches oxmysql directly, around the gate: ' .. line)
        end
    end
    ...
    for _, source in ipairs({ stats, util }) do
        for line in source:gmatch('[^\n]+') do
            if line:find('MySQL%.query%s*%(') and not line:find('^%s*%-%-') then
                error('a server file calls MySQL.query again, which needs the manifest include: ' .. line)
            end
        end
    end
end)
