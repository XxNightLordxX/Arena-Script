...
function ArenaCompat.WarnLateStartOnce()
    ...
    say('  AND IT IS THE FIRST THING TO SUSPECT if fighters come out of a round INVISIBLE TO')
    say('    EACH OTHER. A down state entered by one of those scripts is theirs to clear, and')
    say('    the arena stands the casualty straight back up underneath it -- so their cleanup')
    say('    never runs and whatever it did to that player stays done. The tell is that the')
    say('    player can still see THEMSELVES and a relog fixes it: this resource writes')
    say('    nothing that survives a reconnect, and it never touches another player\'s ped at')
    say('    all -- SetEntityVisible here is only ever called on the local one, which would')
    say('    hide them from themselves too. Reported off a live server, and it was this.')
