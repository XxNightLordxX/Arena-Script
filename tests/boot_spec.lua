...
--- THE ALLOW-LIST, WHEREVER IT IS.
---
--- .luacheckrc used to sit beside fxmanifest.lua and was removed with the
--- test suite when the resource was stripped for production -- it is a
--- developer file, and a shipped resource has no lint config. The list itself
--- is not a developer file: it is the only written statement of which globals
--- this resource is allowed to read and which it defines, and this spec is
--- what holds the code to it. So the copy lives here now, and the shipped one
--- is still preferred if an operator's tree has it -- restore .luacheckrc and
--- this reads that instead, with nothing here to change.
local luacheck = (function()
    local shipped = io.open('../Crimson-Arena/.luacheckrc', 'r')
    if shipped then
        shipped:close()
        return Sandbox.readDeclarations('../Crimson-Arena/.luacheckrc')
    end
    return Sandbox.readDeclarations('fixtures/luacheckrc.lua')
end)()
...
local function boot(realm, overrides)
    ...
    for _, path in ipairs(Sandbox.realmScripts(manifest, realm)) do
        ...
        Sandbox.loadInto('../Crimson-Arena/' .. path, env)
    end
