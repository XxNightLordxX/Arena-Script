...
t.test('the shipped catalogue is rich enough for the rest of this file to mean anything', function()
    ...
    -- THE TEST BELOW SWITCHES ITS OWN OFF, so it does not care how many ship
    -- disabled -- and it must not, because that number is the operator's to
    -- choose. It was 0 when this was written and is 1 now, the musket having
    -- been switched off at the owner's instruction. Asserting a fixed number
    -- here made a config decision fail a test that is not about config, which
    -- is exactly the "test whose subject a config edit deletes" this comment
    -- used to warn about. Recorded, not asserted.
    t.isTrue(disabled >= 0,
        'the shipped catalogue could not be read at all')
...
t.test('nothing an operator has disabled appears in the snapshot', function()
    ...
    -- THE FIXTURE'S OWN, PLUS WHATEVER SHIPS OFF. Every disabled weapon was
    -- checked above, and the shipped catalogue may legitimately carry some of
    -- its own -- the musket does. What this line is for is proving the loop
    -- ran at all, so it asserts the fixture's are included rather than that
    -- nothing else is.
    t.isTrue(checked >= #OFF,
        ('the fixture switched off %d but only %d disabled weapons were checked')
            :format(#OFF, checked))
...
local function panelConfigReads()
    local handle = assert(io.open('../Crimson-Arena/html/app.js', 'r'), 'cannot open html/app.js')
