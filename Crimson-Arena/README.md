...
- `/arenaadmin` lists who owes what, and says on the same line which of three things it is doing: writing to the database, holding it in memory only, or — the one worth catching — `Config.Database.enabled` on but the database refusing the arena, which reads as saved to nobody.
...
If you run `sc-dispatch`, none of this needs writing and there is nothing to paste: it ships its own arena integration. Set `Config.Integrations.CrimsonArena = true` in **its** config and it asks this resource — on the player's own client, before a shots-fired, person-down, person-dead or help-call alert is raised at all.

**That covers police alerts and not medical ones.** `sc-ambulance` is a separate resource with no arena integration at all: it raises `hospital:server:EMSDownAlert` from two places and `hospital:server:ambulanceAlert` from three, and the handler for the first calls `sc-dispatch`'s **server** export directly — already past the client check above. `cancelEvents` cannot help either, because `sc-ambulance` never calls `WasEventCanceled()`. On its shipped config, with `sc-dispatch` running, `EMSDownAlert` is the only one of the two that fires at all, and its calls are at least withdrawn by the retract layer's sweep when the fighter is revived — `emsdown_%d_%d` is one of the shipped `idTemplates` — so they land on a medic's screen and then clear, rather than never landing. (The instant route, withdrawing off `sc-dispatch`'s own announcement, ships off: `Config.Dispatch.custom.retract.filedEvent` is empty because `sc-dispatch` accepts alert payloads from any client on its stock config.) The three `ambulanceAlert` sites are dormant `else` branches today, and the day a setting changes they go straight to every on-duty medic with no call id and nothing to withdraw. Two lines at the top of those two handlers close both.

**Both halves live in one file: [`DISPATCH-ALERTS.md`](../DISPATCH-ALERTS.md)** — what to set, what to paste, where it goes, and what the one setting on this side is that can break the first half. The admin tablet's **Tools → Police & EMS** page reports whether each half took.
...
- `slots` caps how many weapons are honoured. Entries past the cap, and entries of a kind the server switched off, are dropped and named in the rejection list — the request still succeeds.
