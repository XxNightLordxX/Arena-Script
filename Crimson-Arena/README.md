...
**That covers police alerts and not medical ones.** `sc-ambulance` is a separate resource with no arena integration at all: it raises `hospital:server:ambulanceAlert` from six places and `hospital:server:EMSDownAlert` from two, and the handler for the second calls `sc-dispatch`'s **server** export directly — already past the client check above. `cancelEvents` cannot help either, because `sc-ambulance` never calls `WasEventCanceled()`. The two paths are not equally bad: the `EMSDownAlert` one files through `AddNotification`, so the retract layer withdraws it a quarter of a second later, while the six `ambulanceAlert` sites go straight to every on-duty medic with no call id and nothing to withdraw. Two lines at the top of those two handlers close both.
