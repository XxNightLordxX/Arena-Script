...
t.test('a revive stands a dead player up and gives them everything back', function()
    local f = newDispatch()
    f.dead = true

    f.D.Revive(nil)

    t.equals(#f.resurrects, 1, 'a dead player was not resurrected')
    -- Same two flags as the mid-death clear: the revive keeps their gear
    -- and leaves the health to the line below it, which reads the number
    -- the caller asked for.
    t.equals(f.resurrects[1].keepGear, true, 'the revived player lost their loadout')
    t.equals(f.resurrects[1].resetHealth, false, 'the resurrect reset health over the caller\'s number')
    t.equals(f.entity.invincible, false, 'the revived player was left invincible')
    t.equals(f.entity.visible, true, 'the revived player was left invisible')
    t.equals(f.entity.visibleNetwork, false, 'the revive was replicated to every other player')
    t.equals(f.entity.collision, true, 'the revived player was left walking through walls')
    t.equals(f.entity.frozen, false, 'the revived player was left frozen')
end)

t.test('but a revive that KEEPS THE HOLD touches none of those four', function()
    -- THE ELIMINATION PATH. An eliminated fighter is revived so they leave
    -- the medical script's casualty list while the round carries on
    -- without them -- and then stays held. Release here and the parked
    -- body is left mortal and killable, or with spectate off the
    -- eliminated player simply stands back up, armed and solid, in a live
    -- round.
    local f = newDispatch()
    f.dead = true
    ...
    t.equals(#f.resurrects, 1, 'the eliminated fighter was not taken off the casualty list')
    t.isNil(f.entity.invincible, 'an eliminated fighter was made mortal in a live round')
    t.isNil(f.entity.visible, 'an eliminated fighter was made visible in a live round')
    t.isNil(f.entity.collision, 'an eliminated fighter was given their collision back mid-round')
    t.isNil(f.entity.frozen, 'an eliminated fighter was unfrozen mid-round')
end)

t.test('a player who was WRITHING counts as hurt, even at full health', function()
    -- A medical script's bleed-out puts the player on the floor writhing
    -- and often at FULL health -- it is a state, not an injury. A check
    -- for "dead or hurt" walks straight past the one case the assert
    -- window exists to undo.
    local f = newDispatch()
    f.dead = false
    f.writhing = true
    f.health = 200
    ...
    t.isTrue(f.entity.tasksCleared, 'a player writhing at full health was left writhing')
    t.isTrue(f.entity.effectsStopped, 'the bleed-out screen effects were left running')
end)

t.test('and a player who was fine is not put through any of it', function()
    -- The control. Every one of these is felt by the player, and running
    -- them on somebody who was never hurt stops screen effects other
    -- resources own.
    local f = newDispatch()
    f.dead = false
    f.writhing = false
    ...
    t.isNil(f.entity.tasksCleared, 'an unhurt player had their tasks cleared')
    t.isNil(f.entity.effectsStopped, 'an unhurt player had every screen effect on their client stopped')
    t.equals(#f.resurrects, 0, 'a living player was resurrected')
end)
...
t.test('a revive command from the server is run', function()
    local f = newDispatch()

    t.isTrue(f.fire('crimson_arena:client:runCommand', 'revive 1'))

    t.equals(#f.commands, 1, 'the operator\'s revive command was never run')
    t.equals(f.commands[1], 'revive 1')
end)

t.test('and anything that is not a real command string is refused', function()
    -- A net event handler is reachable by anything the server sends, and
    -- a non-string here would take the handler down.
    local f = newDispatch()

    for _, bad in ipairs({ '', 42, {}, true }) do
        t.isTrue(f.fire('crimson_arena:client:runCommand', bad),
            ('the command handler raised on %s'):format(tostring(bad)))
    end
    f.fire('crimson_arena:client:runCommand', nil)

    t.equals(#f.commands, 0, 'rubbish was executed as a console command')
end)
