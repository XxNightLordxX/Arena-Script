...
local function snapshotMatches()
    ...
    for _, match in ipairs(ArenaLobby.All()) do
        ...
        out[#out + 1] = {
            ...
            -- AND THE SAME ANSWER FOR SOMEBODY FIGHTING IN IT, which is a
            -- different answer: their book shuts the moment the round goes
            -- live. One snapshot goes to the whole room, so both have to ride
            -- on it and the panel picks the one that is about the reader.
...
--- @param ejected boolean? -- the server removed them, and they earned it:
---        treated like a disconnect at every door EXCEPT the record, which
---        they wear. See the stats branch below for why the two differ.
function ArenaLobby.Leave(src, reasonKey, dropped, ejected)
    ...
    if wasFighting and Arena.IsKey(leftName) then
        -- THREE EXITS, THREE SENTENCES. `ejected` rides as `dropped`
        -- everywhere that money and refusals are decided -- which is right --
        -- so without this line the room was told a fighter the SERVER threw
        -- out had lost their connection.
...
function ArenaLobby.UpdateMatch(src, data)
    ...
    -- AND THE RULES OF THE ROUND ARE LOCKED THE MOMENT ANYBODY HAS PAID TO
    -- BE IN IT, which is the bigger half of the same idea and was missing.
    --
    -- The mode lock above asks about the BOOK. It says nothing about the
    -- people who paid an entry fee, and those are the ones being played:
    -- everything below could be rewritten after the room had filled and
    -- every stake was escrowed. What that bought, on the shipped panel with
    -- no modified client:
    --
    --   open an even-looking round -- last man standing, three lives, an
    --   hour on the clock -- let eight people pay 50,000 each, then set the
    --   win condition to a kill limit of ONE and start. The first kill takes
    --   the entire pot. `scoreLimit.min` is 1, so the panel itself offers it.
    --
    --   or shorten a gun-game ladder to its two-rung minimum, which is two
    --   kills off one accomplice.
    --
    --   or move the round to a different arena, halve the lives, or take the
    --   radar away, after everyone had agreed to the ones on the card.
    --
    -- So: a lobby nobody has paid into stays as editable as it ever was, and
    -- one somebody has paid into is the round they paid for. The host's way
    -- out is the one they already have -- close the lobby, which refunds
    -- every stake and costs them the room.
    --
    -- The ENTRY FEE was already locked for exactly this reason. This closes
    -- the rest of the card behind the same line.
