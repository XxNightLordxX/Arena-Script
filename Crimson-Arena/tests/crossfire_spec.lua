...
t.test('and the arena in the sky is guarded the same way', function()
    ...
    -- the check is a sphere too -- it was x/y only, and that drew the arena
    -- on the desert floor beneath it; see the ground-under test below. A
    -- grenade thrown at the trailer park's coordinates must not be refused
    -- for being inside the skydome, and one thrown at the skydome's must be.
    local f = newFixture()
    f.enter(1, 'm1')
...
t.test('but an eliminated BODY is still shootable, so it cannot be used as a shield',
    function()
        -- THE OTHER END OF THIS RULE IS NOT THE SAME RULE, and treating it as
        -- one made a corpse into cover.
        --
        -- The guard exists because an eliminated fighter keeps their place in
        -- the match's own instance to watch, so their damage at a live
        -- fighter was landing. That is the exploit: they gain nothing on the
        -- board and can delete whoever is winning. Refusing shots AT them
        -- looked like the same sentence read backwards and is not.
        --
        -- With spectateOnElimination on -- the shipped setting -- their body
        -- stays standing where it died, in the middle of the arena. One
        -- shotgun blast or grenade is ONE damage packet naming everybody it
        -- touched, and a crossfire refusal cancels the packet WHOLE: so a
        -- spread that clipped that body did nothing to any of the live
        -- fighters beside it. A weapon that visibly connects and does nothing
        -- is worse than the thing this was guarding against.
        --
        -- Nothing is lost by letting it through: they are already out, and
        -- resolveKiller refuses to credit a kill on an eliminated victim.
        local f = newFixture()
        f.enter(1, 'm1')
        f.enter(2, 'm1')
        local roster = f.teams('m1', 'tdm', { [1] = 'crimson', [2] = 'ash' })
        ...
        t.isFalse(f.shoot(1, { 2 }),
            'an eliminated body still cancels the packet, so it works as a shield')
    end)
...
t.test('DEFECT: an arena with no fence had no explosion guard at all', function()
    ...
    -- explosionEvent are gated on the point test, so on a nil boundary the
    -- guard did not merely get the circle wrong, it stopped running.
    local f = newFixture()
    f.env.Config.Arenas.trailerpark.boundary.enabled = false
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'trailerpark' } end }

    t.isTrue(f.explode(9, PARK.x, PARK.y, PARK.z),
        'an outsider shelled a live round because the arena had no fence')
end)

t.test('and the round being fought in it can still use its own explosives', function()
    -- The other direction, and the one a careless fix breaks: closing the hole
    -- must not start refusing a fighter's own grenade in his own round.
    local f = newFixture()
    f.env.Config.Arenas.trailerpark.boundary.enabled = false
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'trailerpark' } end }

    t.isFalse(f.explode(1, PARK.x, PARK.y, PARK.z),
        'a fighter could not use a grenade in his own open arena')
end)

t.test('and an explosion nowhere near an open arena is still nobody\'s business', function()
    -- The fallback radius must not swallow the map. It is the spawn ring with
    -- Config.Match.maxKillDistance as a floor, not an unbounded circle.
    local f = newFixture()
    f.env.Config.Arenas.trailerpark.boundary.enabled = false
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'trailerpark' } end }
    ...
    t.isFalse(f.explode(9, PARK.x + 5000.0, PARK.y + 5000.0, PARK.z),
        'an explosion on the far side of the map was cancelled by an open arena')
end)


t.test('DEFECT: the sky arena cancelled explosions on the ground a kilometre beneath it', function()
    -- The boundary is a sphere a kilometre up. Measured on x and y alone it
    -- was a circle drawn on the desert floor, and every bystander explosion
    -- inside that circle -- in a live round they could not see -- was
    -- cancelled as if it had landed on the platform.
    local f = newFixture()
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'skydome' } end }

    t.isFalse(f.explode(9, SKY.x, SKY.y, 30.0),
        'an explosion on the ground under the sky arena was cancelled')
    t.isFalse(f.explode(9, SKY.x + 100.0, SKY.y, 30.0),
        'an explosion on the ground a hundred metres sideways under it was cancelled')
end)

t.test('and an explosion ON the platform is still refused, which is the control', function()
    local f = newFixture()
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'skydome' } end }

    t.isTrue(f.explode(9, SKY.x, SKY.y, SKY.z), 'an outsider shelled the platform and was not refused')
    t.isTrue(f.explode(9, SKY.x, SKY.y, SKY.z - 50.0), 'fifty metres below the platform is inside the sphere')
end)

t.test('and a fighter\'s own grenade on the platform is still allowed', function()
    local f = newFixture()
    f.enter(1, 'm1')
    f.env.ArenaLobby = { Get = function() return { arenaKey = 'skydome' } end }
